...
def _cookie_security_flags(response) -> list:
    """Extract security attributes from each raw Set-Cookie header."""
    raw_headers = getattr(getattr(response, 'raw', None), 'headers', None)
    if raw_headers is not None and hasattr(raw_headers, 'getlist'):
        set_cookie_headers = raw_headers.getlist('Set-Cookie')
    else:
        header = response.headers.get('Set-Cookie', '')
        set_cookie_headers = [header] if header else []
...
def _grade_200_response(path: str, body: str, size: int) -> tuple:
    ...
    ALWAYS_CRITICAL_PATHS = {
        '/.git/config': [r'(?i)\[core\]\s*repositoryformatversion'],
        '/.git/HEAD': [r'(?i)ref: refs/heads/'],
        '/.env': [r'(?i)(APP_ENV|DB_CONNECTION|APP_KEY)'],
        '/.env.local': [r'(?i)(APP_ENV|DB_CONNECTION|APP_KEY)'],
        '/.env.production': [r'(?i)(APP_ENV|DB_CONNECTION|APP_KEY)'],
        '/wp-config.php': [r'(?i)define\s*\(\s*\'DB_NAME\''],
        '/config.php': [r'(?i)<\?php'],
        '/configuration.php': [r'(?i)(class JConfig|public \$db)'],
        '/settings.php': [r'(?i)(databases|\$databases)'],
        '/web.config': [r'(?i)<configuration>'],
        '/phpinfo.php': [r'(?i)phpinfo\(\)'],
        '/info.php': [r'(?i)phpinfo\(\)'],
        '/server-status': [r'(?i)Apache Server Status'],
        '/server-info': [r'(?i)Apache Server Information'],
        '/backup.sql': [r'(?i)(CREATE TABLE|INSERT INTO)'],
        '/database.sql': [r'(?i)(CREATE TABLE|INSERT INTO)'],
        '/db_backup.sql': [r'(?i)(CREATE TABLE|INSERT INTO)'],
        '/secrets.txt': [r'(?i)(password|secret)'],
        '/credentials.txt': [r'(?i)(password|username)'],
        '/package.json': [r'(?i)"name"\s*:\s*".+"'],
        '/composer.json': [r'(?i)"require"\s*:'],
        '/package-lock.json': [r'(?i)"lockfileVersion"'],
        '/yarn.lock': [r'(?i)# THIS IS AN AUTOGENERATED FILE']
    }
    if path in ALWAYS_CRITICAL_PATHS:
        for pattern in ALWAYS_CRITICAL_PATHS[path]:
            if re.search(pattern, body_sample):
                return ('critical', 'Inherently sensitive path returned 200', '')
        return (None, None, None) # Content didn't match the expected sensitive format
...
def web_vuln_scan(target: str, use_ssl: bool = False, mutate: bool = False, workers: int = 20) -> dict:
    ...
    def check(path):
        ...
        # Fix: Catch-all routing handling. If the server returns 200 for a known non-existent path, it's a catch-all.
        # We handle this by checking if the response body matches the catch-all response body.
        non_existent_r = _get(base, f"/this_does_not_exist_{random.randint(10000, 99999)}", allow_redirects=True)
        is_catch_all = non_existent_r and non_existent_r.status_code == 200

        if is_catch_all and r.status_code == 200:
            # If it's a catch-all and we get a 200, we must verify the content is not the catch-all page.
            if len(r.content) == len(non_existent_r.content) and r.text == non_existent_r.text:
                return None  # It's just the catch-all page.
...
def http_header_audit(target: str, use_ssl: bool = False) -> dict:
    ...
    cookies = _cookie_security_flags(r)
    for cookie in cookies:
        cookie_name = cookie['name']
        has_secure = cookie['secure']
        has_httponly = cookie['httponly']
    ...
    return {'missing': missing, 'leaks': leaks, 'cookies': cookies, 'score': score}
    ...
    for tech, version in detected_versions.items():
        if tech in CVE_DATABASE:
            ...
            if matched_cves:
                cve_results[tech] = {
                    "version": version,
                    "cves": list(matched_cves),
                    "confidence": "version_match_candidate",
                    "note": "Verify vendor advisories and backported patches before treating as confirmed.",
                }
                alert(f"{BR}{R}[!] {tech} {version} may match: {', '.join(matched_cves)}{RS}")
            else:
